...
def _score_qualite_prix(df: pd.DataFrame) -> pd.Series:
    ...
    """
    df = df.copy()
    df["prix_m2"] = df["prix"].astype(float) / df["surface"].replace(0, 1)
    ...
    return (ratio_score + equip_score).clip(0, 10)
...
def _score_prediction(df: pd.DataFrame) -> pd.Series:
    ...
    df_copy = df.copy()
    df_copy["type_enc"] = le_type.fit_transform(df_copy["type"])
    df_copy["ville_enc"] = le_ville.fit_transform(df_copy["ville"])
...
def _compute_all_scores(db: Session):
    ...
    qp = _score_qualite_prix(df)
    dem = _score_demande(df)
    pred = _score_prediction(df)
    ...
    for i, row in df.iterrows():
        results.append({
            ...
            "score_total": float(score_total.iloc[i]),
            "detail": {
                "qualite_prix": round(float(qp.iloc[i]), 1),
                "demande_zone": round(float(dem.iloc[i]), 1),
                "prediction": round(float(pred.iloc[i]), 1),
            }
        })
...
def score_bien(bien_id: int, db: Session = Depends(get_db)):
    ...
    df = _get_biens_df(db)
    row = df[df["id"] == bien_id].iloc[0]
    prix_m2 = float(row["prix"]) / max(float(row["surface"]), 1)
    ville_df = df[df["ville"] == row["ville"]]
    ville_avg = float(ville_df["prix"].astype(float).mean() /
                       ville_df["surface"].replace(0, 1).astype(float).mean())
    ...
    return {
        ...
        "detail": {
            "qualite_prix": {
                "score": qp_score,
                "prix_m2_bien": round(prix_m2, 2),
                "prix_m2_moyen_ville": round(ville_avg, 2),
            ...
            "prediction": {
                ...
                "interpretation": "Le modèle estime que ce bien est " + (
                    "sous-évalué (bonne affaire)" if pred_score > 6
                    else "correctement évalué" if pred_score > 4
